...
class Backbone(nn.Module):
    ...
    def __init__(self,
        ...
        self.n2n_h_attn_layers = nn.ModuleList([GraphAttention(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout, has_edge_attr=True, if_self_attention=True) for _ in range(num_attn_layers-1)])
        self.n2n_a_attn_layers = nn.ModuleList([GraphAttention(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout, has_edge_attr=True, if_self_attention=True) for _ in range(num_attn_layers-1)])
        self.n2n_s_attn_layers = nn.ModuleList([GraphAttention(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout, has_edge_attr=False, if_self_attention=True) for _ in range(num_attn_layers-1)])
    ...
    def forward(self, data: Batch, l_embs: torch.Tensor) -> torch.Tensor:
        ...
        # inverse learning attention edge_index
        ...
        edge_index = torch.stack([(original_tensor[:, 1:]).repeat_interleave(rows),original_tensor.T[((original_tensor[:, 1:]) - 1) % cols].flatten()], dim=0)
        ...
        edge_index = edge_index[:, a_mask[edge_index[0] // cols, edge_index[0] % cols] & a_mask[edge_index[1] // cols, edge_index[1] % cols]]
        ...
        m_batch = a_batch.unsqueeze(1).repeat_interleave(self.num_modes,1)                       # [(N1,...,Nb),K]
        ...
        m_valid_mask = a_mask.unsqueeze(2).repeat_interleave(self.num_modes,2)  #[(N1,...,Nb),H,K]
